...
1. Filter `spotify` to just songs in a minor key, then
2. Select the `loudness` and `energy` columns from the result

```{r echo = TRUE}
minor_songs <- filter(spotify, mode == 0)
minor_songs <- select(minor_songs, loudness, energy)
minor_songs
```

### Redundancy
 Do you notice how we re-create `minor_songs` at each step so we will have something to pass to the next step? This is an inefficient way to write R code.

You could avoid creating `minor_songs` by nesting your functions inside of each other, but this creates code that is hard to read:

```{r echo = TRUE, eval = FALSE}
select(filter(spotify, mode == 0), loudness, energy)
```
